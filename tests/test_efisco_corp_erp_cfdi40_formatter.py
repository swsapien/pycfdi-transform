...
class TestEfiscoCorpERPCFDI40Formatter(unittest.TestCase):

    def test_columns_names_cfdi40(self):
        formatter = EfiscoCorpERPCFDI40Formatter({'cfdi40': {}})
        columns_expected = ['VERSION', 'SERIE', 'FOLIO', 'FECHA', 'NOCERTIFICADO', 'SUBTOTAL', 'DESCUENTO', 'TOTAL', 'MONEDA', 'TIPOCAMBIO', 'TIPODECOMPROBANTE', 'METODOPAGO', 'FORMAPAGO', 'CONDICIONESDEPAGO', 'LUGAREXPEDICION', 'EMISORRFC', 'EMISORNOMBRE', 'EMISORREGIMENFISCAL', 'RECEPTORRFC', 'RECEPTORNOMBRE', 'RESIDENCIAFISCAL', 'NUMREGIDTRIB', 'RECEPTORUSOCFDI', 'IVATRASLADO', 'IEPSTRASLADO', 'TOTALIMPUESTOSTRASLADOS', 'ISRRETENIDO', 'IVARETENIDO', 'IEPSRETENIDO', 'TOTALIMPUESTOSRETENIDOS', 'TOTALTRASLADOSIMPUESTOSLOCALES', 'TOTALRETENCIONESIMPUESTOSLOCALES', 'UUID', 'FECHATIMBRADO', 'RFCPROVCERTIF', 'SELLOCFD']
        self.assertListEqual(columns_expected, formatter.get_columns_names())
    
    def test_initialize_class_error_cfdi40(self):
        with self.assertRaises(AssertionError) as ex:
            EfiscoCorpERPCFDI40Formatter(cfdi_data=None)
    ...
    def test_initialize_class_error_version_cfdi40(self):
        with self.assertRaises(AssertionError) as ex:
            EfiscoCorpERPCFDI40Formatter({'cfdi32': {}})
    ...
    def test_formatter_error_tfd_cfdi40(self):
        ...
        cfdi_data.pop('tfd11')
        formatter = EfiscoCorpERPCFDI40Formatter(cfdi_data)
    ...
    def test_formatter_cfdi40_implocal10_ok(self):
        ...
        self.assertEqual(data_columns[0][28],'0.00')
        self.assertEqual(data_columns[0][29],'')
    
    def test_formatter_cfdi40_implocal10_safe_numerics_1(self):
        sax_handler = CFDI40SAXHandler(safe_numerics=True).use_implocal10()
        cfdi_data = sax_handler.transform_from_file(os.path.dirname(__file__) + '/Resources/implocal/cfdi40_01_implocal.xml')
        formatter = EfiscoCorpERPCFDI40Formatter(cfdi_data,empty_char='',safe_numerics=True)
        data_columns = formatter.dict_to_columns()
        self.assertTrue(formatter.can_format())
        self.assertEqual(formatter.get_errors(),'')
        self.assertTrue(len(data_columns) == 1)
    ...
    def test_formatter_cfdi40_implocal10_safe_empty_char_1(self):
        sax_handler = CFDI40SAXHandler().use_implocal10()
        cfdi_data = sax_handler.transform_from_file(os.path.dirname(__file__) + '/Resources/implocal/cfdi40_01_implocal.xml')
        formatter = EfiscoCorpERPCFDI40Formatter(cfdi_data,empty_char='-',safe_numerics=False)
        data_columns = formatter.dict_to_columns()
        self.assertTrue(formatter.can_format())
        self.assertEqual(formatter.get_errors(),'')
        self.assertTrue(len(data_columns) == 1)
    ...
    def test_formatter_cfdi40_addenda(self):
        ...
        formatter = EfiscoCorpERPCFDI40Formatter(cfdi_data, safe_numerics=True, empty_char='-')
        self.assertTrue(formatter.can_format())
        self.assertEqual(formatter.get_errors(), '')
